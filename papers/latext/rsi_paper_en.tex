...
in under 1\,ms versus 683--1082\,ms for full model retraining---at
least a $600\times$ speedup.
...
\begin{enumerate}[leftmargin=2em, label=(\arabic*)]
  ...
  \item \textbf{Empirical validation} across five experiments
        demonstrating zero-shot performance, a $600\times$ update
...
XGBoost$^\dagger$ & 0.967 & 0.999 & 0.978 & Yes & 683--1082\,ms \\
...
\midrule
RSI (ours) & \textbf{$<$1\,ms} & 0.411 & No \\
XGBoost & 683--1082\,ms & 0.954 & Yes \\
MLP & 61\,ms & 0.000 & Yes \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Note on Update Efficiency.} The speedup specifically measures the computational cost of \textbf{absorbing a regulatory update} (i.e., re-calculating the posterior for a new rule parameter) compared to a full model retraining on the entire dataset $\mathbf{D}$. While single-instance inference latency remains competitive across all models, the $O(1)$ property of RSI provides a critical advantage for institutional systems where regulations change annually or across thousands of taxpayer categories.

RSI absorbs regulatory updates in under 1\,ms---at least $\mathbf{600\times}$ faster than
XGBoost full retraining (683--1082\,ms across runs on the test machine).
Absolute timings are hardware-dependent; the $O(1)$ complexity guarantee
is machine-independent and holds regardless of dataset size or platform.
Over $T$ annual updates (Togo: $T \approx 4$--$6$),
...
RSI posterior uncertainty $\sigma[c_i | \mathbf{D}]$ exhibits a
general decreasing trend with sample size $N$, consistent with the
asymptotic guarantees of Theorem~\ref{thm:bvm}. Minor oscillations
at finite $N$ are expected under mean-field VI with small samples.
For $N < 25$, RSI achieves competitive recall while XGBoost cannot
operate (insufficient labeled examples).
...
within 7 iterations, confirming Theorem~\ref{thm:elbo}. The small
absolute change in ELBO reflects the proximity of the variational
posterior to the prior at convergence, consistent with the
zero-shot regime where labeled data are absent.
...
demonstrate at least a $600\times$ update speedup over retraining-based
methods.
